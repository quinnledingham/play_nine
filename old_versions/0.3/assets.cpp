...
internal Texture
blank_texture(u32 width, u32 height, u32 channels) {
    Texture bitmap = {};
    bitmap.width = width;
    ...
    bitmap.data = (u8 *)malloc(bitmap_size);
...
// deep copy
internal Texture
copy_texture(Texture *src) {
  Texture dest = {};

  dest = *src;

  u32 size = src->width * src->height * src->channels;
  dest.data = (u8*)SDL_malloc(size);
  SDL_memcpy(dest.data, src->data, size);
...
internal void
copy_blend_texture(Texture dest, const Texture src, Vector2_s32 position) {
    if (src.width == 0 || src.height == 0)
        return;

    if (dest.channels != src.channels) {
        app_log_error("copy_blend_bitmap() bitmap channels do not match (%d != %d)\n", dest.channels, src.channels);
        ASSERT(0);
    }

    if (dest.width < src.width || dest.height < src.height) {
        app_log_error("copy_blend_bitmap() src bitmap is too big\n");
        ASSERT(0);
    }
    
    u8 *src_ptr = src.data;
    u8 *dest_ptr = dest.data + (position.x * dest.channels) + (position.y * dest.pitch);
    u8 *dest_ptr_line_start = dest_ptr;
    
    while(src_ptr < src.data + (src.width * src.height * src.channels)) {
        ...
        if (dest_ptr >= dest_ptr_line_start + src.pitch) {
            s32 src_line = u32(src_ptr - src.data) / src.pitch;
            dest_ptr = dest.data + (position.x * dest.channels) + (position.y * dest.pitch) + (src_line * dest.pitch);
...
texture_atlas_init(Texture_Atlas *atlas, u32 layout_id) {
  atlas->texture.sets[0] = vulkan_descriptor_set(layout_id);
  atlas->texture.sets[1] = vulkan_descriptor_set(layout_id);

  for (u32 i = 0; i < Vulkan_Context::max_frames_in_flight; i++) {
    atlas->texture.textures[i] = atlas->texture.src;

    vulkan_destroy_texture(&atlas->texture.textures[i]);
    vulkan_create_texture(&atlas->texture.textures[i], TEXTURE_PARAMETERS_CHAR);

    Descriptor texture_desc = vulkan_descriptor(&atlas->texture.sets[i], 0);
    vulkan_set_texture(&texture_desc, &atlas->texture.textures[i]);
  }
}
...
create_texture_atlas(u32 width, u32 height, u32 channels) {
  Texture_Atlas atlas = {};
  atlas.texture.src = blank_texture(width, height, channels);
...
texture_atlas_reset(Texture_Atlas *atlas) {
    u32 bitmap_size = atlas->texture.src.width * atlas->texture.src.height * atlas->texture.src.channels;
    memset(atlas->texture.src.data, 0, bitmap_size);
...
internal u32
texture_atlas_add(Texture_Atlas *atlas, Texture *bitmap) {
  Vector2_s32 padding = { 1, 1 };
  
  Vector2_s32 position = {};

  // Check if bitmap fits next in line
  if (atlas->insert_position.x + bitmap->width  + padding.x < atlas->texture.src.width &&
    atlas->insert_position.y + bitmap->height + padding.y < atlas->texture.src.height) {
    position = atlas->insert_position;
  // Check if the bitmap fits on the next line
  } else if (atlas->insert_position.y + atlas->row_height + bitmap->height + padding.y < atlas->texture.src.height) {
    position = { 0, atlas->insert_position.y + atlas->row_height + padding.y };
    atlas->row_height = 0;
  // No more space reset atlas
  } else {
    texture_atlas_reset(atlas);
    app_log_error("texture_atlas_add() not enough room for bitmap\n");
  }

  copy_blend_texture(atlas->texture.src, *bitmap, position);

  Vector2 tex_coords_p1 = { 
    (float32)position.x / (float32)atlas->texture.src.width, 
    (float32)position.y / (float32)atlas->texture.src.height 
  };
  Vector2 tex_coords_p2 = { 
    float32(position.x + bitmap->width)  / (float32)atlas->texture.src.width, 
    float32(position.y + bitmap->height) / (float32)atlas->texture.src.height 
  };
  ...
  for (u32 i = 0; i < Vulkan_Context::max_frames_in_flight; i++) {
      atlas->texture.refresh_required[i] = true;
  }
...
texture_atlas_refresh(Texture_Atlas *atlas) {
  u32 i = vk_ctx.current_frame;
  atlas->texture.sets[i].texture_index = 0;
  if (atlas->texture.refresh_required[i]) {
    atlas->texture.refresh_required[i] = false;

    atlas->texture.textures[i] = copy_texture(&atlas->texture.src);
...
load_font_char_bitmap(Font *font, u32 codepoint, float32 scale) {
  if (scale == 0.0f) {
      app_log_error("load_font_char_bitmap() scale can not be zero"); // scale used below
      return 0;
  }
  ...
  char_bitmap->bitmap.data = stbtt_GetGlyphBitmapSubpixel(info, 0, char_bitmap->scale, 0, 0, char_bitmap->font_char->glyph_index, (s32*)&char_bitmap->bitmap.width, (s32*)&char_bitmap->bitmap.height, 0, 0);
...
clear_font_bitmap_cache(Font *font) {
  stbtt_fontinfo *info = (stbtt_fontinfo*)font->info;
  for (s32 i = 0; i < font->cache->bitmaps_cached; i++) {
      stbtt_FreeBitmap(font->cache->bitmaps[i].bitmap.data, info->userdata);
      font->cache->bitmaps[i].bitmap.data = 0;
...
internal s32
load_assets(void *ptr) {
  Assets_Load_Info info = *(Assets_Load_Info*)ptr;
  ...
  for (u32 i = 0; i < info.loads_count; i++) {
    if (load_assets_switch(i, info.asset_type, info.loads) == FAILURE) {
      return 1;
    }
  }
...
internal s32 
init_assets() {
  // seperate function so that on swap chain recreation the pipelines can be re-inited.
  init_pipelines();
