...
internal s32
get_score(Player_Card *cards) {
    s32 score = 0;

    Card_Pair pairs[4] = {
        create_pair(cards[0].index, cards[4].index),
        create_pair(cards[1].index, cards[5].index),
        create_pair(cards[2].index, cards[6].index),
        create_pair(cards[3].index, cards[7].index)
    };
...
update_scores(Game *game) {
    for (u32 i = 0; i < game->players_count; i++) {
        Player *player = &game->players[i];
        Player_Card *cards = player->cards;            
...
fill_total_scores(Game *game) {
    for (s32 player_index = 0; player_index < (s32)game->players_count; player_index++) {
...
get_winner(Game *game) {
    ...
    for(u32 i = 1; i < game->players_count; i++) {
