...
const char* get_filename(const char *filepath) {
  ...
  char *filename = (char*)SDL_malloc(name_length + 1);
...
char_array_to_s32(const char *ptr, s32 *result) {
    ...
    while (SDL_isdigit(*ptr)) num = 10 * num + (*ptr++ - '0');
    *result = sign * num;
...
s32_to_char_array(char *buffer, u32 size, s32 in) {
    u32 ret = SDL_snprintf(buffer, size, "%d", in);
    if (ret < 0) {
        app_log_error("s32_to_char_array(): snprintf() failed\n");
        return 0;
    }
    if (ret >= size)
        app_log_error("s32_to_char_array(): snprintf(): result was truncated\n");
    return ret;
}
...
char_array_to_float32(const char *ptr, float32 *result)
{
    ...
    while (SDL_isdigit(*ptr)) num = 10.0 * num + (double)(*ptr++ - '0');

    // @WARNING . or , based on system keyboard
    if (*ptr == '.' || *ptr == ',') ptr++;

    while (SDL_isdigit(*ptr))
    {
    ...
    if (is_exponent(*ptr))
    {
        ...
        while (SDL_isdigit(*ptr)) eval = 10 * eval + (*ptr++ - '0');
