...
vulkan_check_validation_layer_support() {
  u32 requested_layers_count = ARRAY_COUNT(vk_ctx.validation_layers);
  
  u32 available_layers_count;
  vkEnumerateInstanceLayerProperties(&available_layers_count, nullptr);

  VkLayerProperties *available_layers = (VkLayerProperties *)SDL_calloc(available_layers_count, sizeof(VkLayerProperties));
  vkEnumerateInstanceLayerProperties(&available_layers_count, available_layers);

  bool8 all_layers_found = true;
  for (u32 requested_index = 0; requested_index < requested_layers_count; requested_index++) {
    const char *requested_str = vk_ctx.validation_layers[requested_index];
    
    bool8 layer_found = false;
    for (u32 available_index = 0; available_index < available_layers_count; available_index++) {
      const char *available_str = available_layers[available_index].layerName; 
      if (SDL_strcmp(requested_str, available_str)) {
        layer_found = true;
        break;
      }
    }
...
vulkan_debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT message_severity, VkDebugUtilsMessageTypeFlagsEXT message_type, const VkDebugUtilsMessengerCallbackDataEXT* callback_data, void* user_data) {
	vulkan_log("%s", callback_data->pMessage);
	return VK_FALSE;
}
...
vulkan_begin_single_time_commands(VkCommandPool command_pool) {
  ...
  vkAllocateCommandBuffers(vk_ctx.device, &allocate_info, &command_buffer);
...
internal void
vulkan_end_single_time_commands(VkCommandBuffer command_buffer, VkCommandPool command_pool, VkQueue queue) {
  ...
  vkFreeCommandBuffers(vk_ctx.device, command_pool, 1, &command_buffer);
}
...
internal bool8
vulkan_sdl_load_instance_extensions() {
	...
	vk_ctx.instance_extensions = (const char **)SDL_calloc(count_extensions, sizeof(const char *));

	vk_ctx.instance_extensions[0] =  vk_ctx.extra_instance_extensions[0];

	SDL_memcpy(&vk_ctx.instance_extensions[1], loaded_instance_extensions, count_instance_extensions * sizeof(const char *));
...
vulkan_create_instance() {
  ...
  VkInstanceCreateInfo create_info = {};
	...
	if (vk_ctx.validation_layers_enabled) {
    vulkan_populate_debug_messenger_create_info(debug_create_info);
    create_info.pNext = (VkDebugUtilsMessengerCreateInfoEXT*)&debug_create_info;
    
    create_info.enabledLayerCount = ARRAY_COUNT(vk_ctx.validation_layers);
    create_info.ppEnabledLayerNames = vk_ctx.validation_layers;
	} else {
...
vulkan_check_device_extension_support(VkPhysicalDevice device) {
  ...
  for (u32 required_index = 0; required_index < ARRAY_COUNT(vk_ctx.device_extensions); required_index++) {
    bool8 extension_found = false;
    for (u32 available_index = 0; available_index < available_count; available_index++) {
      if (SDL_strcmp(available_extensions[available_index].extensionName, vk_ctx.device_extensions[required_index])) {
...
vulkan_find_queue_families(VkPhysicalDevice device) {
  Vulkan_Queue_Family_Indices indices;

  Arr<VkQueueFamilyProperties> queue_families;
  vkGetPhysicalDeviceQueueFamilyProperties(device, &queue_families.count, nullptr);
  queue_families.init();
  vkGetPhysicalDeviceQueueFamilyProperties(device, &queue_families.count, queue_families.data);

  for (u32 queue_index = 0; queue_index < queue_families.count; queue_index++) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(device, queue_index, vk_ctx.surface, &present_support);
    if (present_support) {
...
bool8 vulkan_is_device_suitable(VkPhysicalDevice device) {
  bool8 extensions_supported = vulkan_check_device_extension_support(device);

  u32 formats_supported = 0;
  u32 present_modes_supported = 0;
  vkGetPhysicalDeviceSurfaceFormatsKHR(device, vk_ctx.surface, &formats_supported, nullptr);
  vkGetPhysicalDeviceSurfacePresentModesKHR(device, vk_ctx.surface, &present_modes_supported, nullptr);

  Vulkan_Queue_Family_Indices indices = vulkan_find_queue_families(device);
  ...
  return indices.graphics_and_compute_family.found && 
         extensions_supported && // all extensions asked for supported
         formats_supported && // any format supported
         present_modes_supported && // any present mode supportecd
         device_features.samplerAnisotropy;// && 
         //device_properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU;
}

internal void 
vulkan_print_device(VkPhysicalDevice device) {
  VkPhysicalDeviceProperties device_properties;
  vkGetPhysicalDeviceProperties(device, &device_properties);
  ...
  // Nvidia
  if (Vulkan_Vendor_IDs::nvidia == device_properties.vendorID) {
    driver_version = vulkan_get_nvidia_driver_version(device_properties.driverVersion);
  // Intel
  } else if (Vulkan_Vendor_IDs::intel == device_properties.vendorID) {
...
vulkan_pick_physical_device() {
  ...
  for (u32 device_index = 0; device_index < device_count; device_index++) {
    if (vulkan_is_device_suitable(devices[device_index])) {
  ...
  vk_ctx.queue_family_indices = vulkan_find_queue_families(vk_ctx.physical_device);
...
vulkan_create_frames() {
  ...
  command_info.commandBufferCount = vk_ctx.max_frames_in_flight;

  for (u32 frame_index = 0; frame_index < vk_ctx.max_frames_in_flight; frame_index++) {
    Vulkan_Frame *frame = &vk_ctx.frames[frame_index];

    // sync objects
    for (u32 semaphore_index = 0; semaphore_index < ARRAY_COUNT(frame->semaphores); semaphore_index++) {      
      if (vkCreateSemaphore(vk_ctx.device, &semaphore_info, nullptr, &frame->semaphores[semaphore_index]) != VK_SUCCESS) {
    ...
    for (u32 fence_index = 0; fence_index < ARRAY_COUNT(frame->fences); fence_index++) {      
      if (vkCreateFence(vk_ctx.device, &fence_info, nullptr, &frame->fences[fence_index]) != VK_SUCCESS) {
        vulkan_log_error("vulkan_create_sync_objects(): failed to create fences\n");        
      }
    }

    // command buffer
    if (vkAllocateCommandBuffers(vk_ctx.device, &command_info, frame->command_buffers) != VK_SUCCESS) {
...
vulkan_create_descriptor_pool() {
	VkDescriptorPoolSize pool_sizes[] = {
		...
		{ VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 25000 },
		{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, vk_ctx.max_frames_in_flight * 128 },
	};
  ...
  VkResult result = vkCreateDescriptorPool(vk_ctx.device, &pool_info, nullptr, &vk_ctx.descriptor_pool);
...
internal void
vulkan_query_swap_chain_support() {
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(vk_ctx.physical_device, vk_ctx.surface, &vk_ctx.surface_capabilities);
  
  vkGetPhysicalDeviceSurfaceFormatsKHR(vk_ctx.physical_device, vk_ctx.surface, &vk_ctx.formats.count, nullptr);
  ASSERT(vk_ctx.formats.count); // only run this function after a suitable physical device has been picked
  vk_ctx.formats.init();
  vkGetPhysicalDeviceSurfaceFormatsKHR(vk_ctx.physical_device, vk_ctx.surface, &vk_ctx.formats.count, vk_ctx.formats.data);

  vkGetPhysicalDeviceSurfacePresentModesKHR(vk_ctx.physical_device, vk_ctx.surface, &vk_ctx.present_modes.count, nullptr);
  ASSERT(vk_ctx.present_modes.count);
  vk_ctx.present_modes.init();
  vkGetPhysicalDeviceSurfacePresentModesKHR(vk_ctx.physical_device, vk_ctx.surface, &vk_ctx.present_modes.count, vk_ctx.present_modes.data);
}
...
internal void 
vulkan_create_buffer(Vulkan_Buffer *buffer, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties) {
  buffer->size = size;
...
internal void 
vulkan_copy_buffer(Vulkan_Buffer src_buffer, Vulkan_Buffer dest_buffer, VkDeviceSize size, u32 src_offset, u32 dest_offset) {
...
internal void 
vulkan_update_buffer(Vulkan_Context *vk_ctx, Vulkan_Buffer *buffer, void *in_data, u32 in_data_size, u32 offset) {
  Vulkan_Buffer staging_buffer = {};
  vulkan_create_buffer(&staging_buffer, (VkDeviceSize)in_data_size, 
  ...
  vulkan_copy_buffer(staging_buffer, *buffer, staging_buffer.size, 0, offset);
...
internal bool8
vulkan_sdl_init() {
  vulkan_log("initializing vulkan...\n");

  if (vk_ctx.validation_layers_enabled && !vulkan_check_validation_layer_support()) {
    vulkan_log_error("Validation layers requested but not supported\n");
  }
  ...
  if (vk_ctx.validation_layers_enabled) {
	...
	if (SDL_Vulkan_CreateSurface(sdl_ctx.window, vk_ctx.instance, NULL, &vk_ctx.surface) == false) {
		vulkan_log_error("vulkan surface failed being created\n");
		return FAILURE;
	}
  ...
  vulkan_create_buffer(&vk_ctx.static_buffer, VULKAN_STATIC_BUFFER_SIZE, 
  ...
  vulkan_create_buffer(&vk_ctx.dynamic_buffer, VULKAN_STATIC_BUFFER_SIZE, 
                VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, 
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
  
  vulkan_create_buffer(&vk_ctx.static_uniform_buffer,  VULKAN_STATIC_UNIFORM_BUFFER_SIZE, 
                VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, 
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
  
  vulkan_create_buffer(&vk_ctx.dynamic_uniform_buffer, VULKAN_DYNAMIC_UNIFORM_BUFFER_SIZE, 
                VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, 
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

  vulkan_create_buffer(&vk_ctx.storage_buffer, 500, 
                VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, 
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

  vulkan_create_buffer(&vk_ctx.triangle_buffer, 10000, 
                VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, 
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

  vulkan_split_buffer_over_frames(&vk_ctx.dynamic_buffer, vk_ctx.frames, Vulkan_Context::max_frames_in_flight);
...
vulkan_choose_swap_surface_format() {
  ...
  for (u32 target_index = 0; target_index < ARRAY_COUNT(vulkan_target_formats); target_index++) {
    for (u32 i = 0; i < vk_ctx.formats.count; i++) {
      if ((vk_ctx.formats[i].format == vulkan_target_formats[target_index]) && vk_ctx.formats[i].colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
        return vk_ctx.formats[i];
      }
    }
  }

  vulkan_log("(vulkan) choose_swap_surface_format", "Picking default format\n(vulkan) Other formats available:\n");
  for (u32 i = 0; i < vk_ctx.formats.count; i++) {
    vulkan_log("(vulkan) format: %d, color space: %d\n", vk_ctx.formats[i].format, vk_ctx.formats[i].colorSpace);
  }

  return vk_ctx.formats[0];
}
...
vulkan_choose_swap_present_mode() {
  if (vk_ctx.vsync)
    return VK_PRESENT_MODE_FIFO_KHR;
  
  for (u32 i = 0; i < vk_ctx.present_modes.count; i++) {
    if (vk_ctx.present_modes[i] == VK_PRESENT_MODE_IMMEDIATE_KHR) {
      return vk_ctx.present_modes[i];
    }
  }
...
internal VkExtent2D
vulkan_choose_swap_extent() {
  if (vk_ctx.surface_capabilities.currentExtent.width != 0xFFFFFFFF) {
    return vk_ctx.surface_capabilities.currentExtent;
  } else {
    VkExtent2D actual_extent = { (u32)vk_ctx.window_dim.width, (u32)vk_ctx.window_dim.height };
    actual_extent.width  = SDL_clamp(actual_extent.width,  vk_ctx.surface_capabilities.minImageExtent.width,  vk_ctx.surface_capabilities.maxImageExtent.width);
    actual_extent.height = SDL_clamp(actual_extent.height, vk_ctx.surface_capabilities.minImageExtent.height, vk_ctx.surface_capabilities.maxImageExtent.height);
...
vulkan_create_swap_chain() {
  vulkan_query_swap_chain_support();
  VkSurfaceFormatKHR surface_format = vulkan_choose_swap_surface_format();
  VkPresentModeKHR present_mode = vulkan_choose_swap_present_mode();
  VkExtent2D extent = vulkan_choose_swap_extent();
  vk_ctx.swap_chain_extent = extent;

  vulkan_log("extent_w: %d, extent_h: %d\n", extent.width, extent.height);
  vulkan_log("surface_format: %d, surface_color_space: %d\n", surface_format.format, surface_format.colorSpace);

  Arr<VkImage> swap_chain_images;
  swap_chain_images.count = vk_ctx.surface_capabilities.minImageCount + 1;
  if (vk_ctx.surface_capabilities.maxImageCount > 0 && swap_chain_images.count > vk_ctx.surface_capabilities.maxImageCount) {
    swap_chain_images.count = vk_ctx.surface_capabilities.maxImageCount;
  }
  swap_chain_images.init();

  VkSwapchainCreateInfoKHR create_info = {};
  create_info.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
  create_info.surface = vk_ctx.surface;
  create_info.minImageCount = swap_chain_images.count;
  create_info.imageFormat = surface_format.format;
  create_info.imageColorSpace = surface_format.colorSpace;
  create_info.imageExtent = extent;
  create_info.imageArrayLayers = 1;
  create_info.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
  create_info.preTransform = vk_ctx.surface_capabilities.currentTransform;
  ...
  vkGetSwapchainImagesKHR(vk_ctx.device, vk_ctx.swap_chains[0], &swap_chain_images.count, swap_chain_images.data);

  vk_ctx.swap_chain_textures.count = swap_chain_images.count;
	vk_ctx.swap_chain_textures.init();
	for (u32 i = 0; i < vk_ctx.swap_chain_textures.count; i++) {
    vk_ctx.swap_chain_textures[i].format = surface_format.format;
    vk_ctx.swap_chain_textures[i].image = swap_chain_images[i];
    vk_ctx.swap_chain_textures[i].mip_levels = 1;
	}
}

internal u32
vulkan_find_memory_type(u32 type_filter, VkMemoryPropertyFlags properties) {
  VkPhysicalDeviceMemoryProperties memory_properties;
...
internal void
vulkan_create_image(Texture *texture, VkSampleCountFlagBits num_samples, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties) {
  ...
  if (vkCreateImage(vk_ctx.device, &image_info, nullptr, &texture->image) != VK_SUCCESS) {
  ...
  allocate_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
  allocate_info.allocationSize = memory_requirements.size;
  allocate_info.memoryTypeIndex = vulkan_find_memory_type(memory_requirements.memoryTypeBits, properties);
...
internal void
vulkan_create_image_view(Texture *texture, VkImageAspectFlags aspect_flags) {
  ...
  if (vkCreateImageView(vk_ctx.device, &view_info, nullptr, &texture->image_view) != VK_SUCCESS) {
      vulkan_log_error("vulkan_create_image_view() failed to create texture image view\n");
  }
}

internal void
vulkan_create_sampler(Texture *texture, u32 texture_parameters) {
  ...
  sampler_info.maxLod = (float32)texture->mip_levels;

  if (vkCreateSampler(vk_ctx.device, &sampler_info, nullptr, &texture->sampler) != VK_SUCCESS) {
    vulkan_log_error("vulkan_create_sampler() failed to create sampler\n");
  }
}

internal void
vulkan_create_render_image_views() {
  vk_ctx.draw_textures.count = vk_ctx.swap_chain_textures.count;
  vk_ctx.draw_textures.init();

  for (u32 i = 0; i < vk_ctx.swap_chain_textures.count; i++) {
    vulkan_create_image_view(&vk_ctx.swap_chain_textures[i], VK_IMAGE_ASPECT_COLOR_BIT);

    vk_ctx.draw_textures[i].dim = vk_ctx.resolution;
    vk_ctx.draw_textures[i].mip_levels = 1;
    vk_ctx.draw_textures[i].format = vk_ctx.swap_chain_textures[i].format;
    vulkan_create_image(&vk_ctx.draw_textures[i], VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    vulkan_create_image_view(&vk_ctx.draw_textures[i], VK_IMAGE_ASPECT_COLOR_BIT);
    vulkan_create_sampler(&vk_ctx.draw_textures[i], TEXTURE_PARAMETERS_DEFAULT);
  }
}

internal VkFormat
vulkan_find_supported_format(VkFormat *candidates, u32 candidates_count, VkImageTiling tiling, VkFormatFeatureFlags features) {
  for (u32 i = 0; i < candidates_count; i++) {
    VkFormatProperties properties;
    vkGetPhysicalDeviceFormatProperties(vk_ctx.physical_device, candidates[i], &properties);
...
internal VkFormat
vulkan_find_depth_format() {
  VkFormat candidates[3] = { VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT };

  VkFormat format = vulkan_find_supported_format(candidates, 3, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
  return format;
}

internal bool8
vulkan_create_draw_render_pass() {
  VkImageLayout final_layout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
  if (vk_ctx.resolution_scaling)
    //final_layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    final_layout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;

  VkAttachmentDescription color_attachment = {};
  color_attachment.format         = vk_ctx.draw_textures[0].format;
  ...
  depth_attachment.format         = vulkan_find_depth_format();
  ...
  color_attachment_resolve.format         = vk_ctx.draw_textures[0].format;
...
vulkan_create_present_render_pass() {
  VkAttachmentDescription color_attachment = {};
  color_attachment.format         = vk_ctx.swap_chain_textures[0].format;
...
internal void
vulkan_transition_image_layout(VkCommandBuffer command_buffer, Texture *texture, VkImageLayout new_layout) {
  ...
  if (new_layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL) {
      ...
      if (vulkan_has_stencil_component(texture->format)) {
  ...
  if (texture->layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
  ...
  } else if (texture->layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
      ...
      destination_stage = VK_PIPELINE_STAGE_TRANSFER_BIT;    
             ...
             (texture->layout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR      && new_layout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)     ||
             (texture->layout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL && new_layout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR)         ) {
  ...
  } else if (texture->layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL) {
  ...
  } else if (texture->layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL && new_layout == VK_IMAGE_LAYOUT_GENERAL) {
  ...
  } else if (texture->layout == VK_IMAGE_LAYOUT_GENERAL && new_layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
      ...
      vulkan_log_error("vulkan_transition_image_layout(): unsupported layout transition\n");
  }
...
vulkan_create_depth_resources() {
  vk_ctx.depth_texture.format = vulkan_find_depth_format();
  vk_ctx.depth_texture.dim.width = vk_ctx.resolution.width;
  vk_ctx.depth_texture.dim.height = vk_ctx.resolution.height;
  vulkan_create_image(&vk_ctx.depth_texture, vk_ctx.msaa_samples, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  vulkan_create_image_view(&vk_ctx.depth_texture, VK_IMAGE_ASPECT_DEPTH_BIT);
  
  VkCommandBuffer command_buffer = vulkan_begin_single_time_commands(vk_ctx.command_pool);
  vulkan_transition_image_layout(command_buffer, &vk_ctx.depth_texture, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);  
  vulkan_end_single_time_commands(command_buffer, vk_ctx.command_pool, vk_ctx.graphics_queue);
}

internal void
vulkan_create_color_resources() {
  vk_ctx.color_texture.format = vk_ctx.draw_textures[0].format;
  vk_ctx.color_texture.dim.width = vk_ctx.resolution.width;
  vk_ctx.color_texture.dim.height = vk_ctx.resolution.height;
  vulkan_create_image(&vk_ctx.color_texture, vk_ctx.msaa_samples, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  vulkan_create_image_view(&vk_ctx.color_texture, VK_IMAGE_ASPECT_COLOR_BIT);
}

internal VkSampleCountFlagBits
vulkan_get_max_usable_sample_count() {
    VkSampleCountFlags counts = vk_ctx.physical_device_properties.limits.framebufferColorSampleCounts & 
                                vk_ctx.physical_device_properties.limits.framebufferDepthSampleCounts;
...
internal bool8
vulkan_create_draw_framebuffers() {  
  vk_ctx.draw_framebuffers.count = vk_ctx.swap_chain_textures.count;
  vk_ctx.draw_framebuffers.init();
  ...
  for (u32 i = 0; i < vk_ctx.draw_framebuffers.count; i++) {  
    if (vk_ctx.anti_aliasing) {
...
internal bool8
vulkan_create_swap_chain_framebuffers() {
  vk_ctx.swap_chain_framebuffers.count = vk_ctx.swap_chain_textures.count;
  vk_ctx.swap_chain_framebuffers.init();
  ...
  for (u32 i = 0; i < vk_ctx.swap_chain_framebuffers.count; i++) {
...
vulkan_create_frame_resources() {
  ...
  if (vk_ctx.anti_aliasing)
    vk_ctx.msaa_samples = vulkan_get_max_usable_sample_count();
  else
  ...
  if (vk_ctx.resolution_scaling) {
    VkExtent2D extent = {};
    extent.width = vk_ctx.resolution.width;
    extent.height = vk_ctx.resolution.height;
...
vulkan_destroy_texture(Texture *texture) {
  ...
  vkFreeMemory(vk_ctx.device, texture->memory, nullptr);
}
...
internal void
vulkan_end_frame() {
  if (!vk_ctx.recording_frame) {
    vulkan_log("Tried to end frame when no frame was being recorded\n");
    return;
  }
  ...
  // Draw Render Pass
  if (vk_ctx.resolution_scaling && !vk_ctx.window_resized) {
    ...
    Texture *texture = &vk_ctx.swap_chain_textures[vk_ctx.image_index];
    vulkan_transition_image_layout(VK_CMD, texture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);  
    ...
    vulkan_transition_image_layout(VK_CMD, texture, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);  
  }
...
pipeline_print_filenames(Pipeline *pipe) {
  ...
  for (u32 i = 0; i < SHADER_STAGES_COUNT; i++) {
    ...
    if (file->filename) {
      s32 written = SDL_snprintf(pos, buffer + sizeof(buffer) - pos, "%s, ", file->filename);
      if (written > 0 && pos + written < buffer + sizeof(buffer)) {
        pos += written;
      }
    }
  }
...
vulkan_convert_shader_stage(u32 shader_stage) {
  switch(shader_stage) {
    ...
    default: {
      vulkan_log_error("vulkan_convert_shader_stage(): %d is not a render shader stage\n", shader_stage);
      return 0;
    }
  };
}
...
vulkan_convert_descriptor_type(u32 descriptor_type) {
  ...
  switch(descriptor_type) {
    ...
    default: {
      vulkan_log_error("vulkan_convert_descriptor_type()", "%d is not a render descriptor type\n", descriptor_type);
    } break;
  }
...
vulkan_create_graphics_pipeline(Vulkan_Context *vk_ctx, Shader *shader, VkRenderPass render_pass) {
  ...
  SDL_memset(shader_stages, 0, sizeof(VkPipelineShaderStageCreateInfo) * SHADER_STAGES_COUNT);
...
vulkan_descriptor_set(u32 gfx_layout_id) {
  GFX_Layout *layout = &vk_ctx.layouts[gfx_layout_id];
...
internal Descriptor
vulkan_descriptor(Descriptor_Set *set, u32 binding_index) {
  Descriptor desc = {};
  desc.set = set;
...
vulkan_bind_descriptor_set(Descriptor_Set set) {
  Pipeline *pipeline = find_pipeline(vk_ctx.active_shader_id);
...
inline void
vulkan_bind_pipeline(u32 id) {
  vk_ctx.active_shader_id = id;
...
inline void 
vulkan_bind_compute_pipeline(u32 id) {
  Pipeline *pipeline = find_pipeline(id);
  vk_ctx.active_shader_id = id;
...
internal void
vulkan_generate_mipmaps(Texture *texture) {
  // Check if image format supports linear blitting
  VkFormatProperties format_properties;
  vkGetPhysicalDeviceFormatProperties(vk_ctx.physical_device, texture->format, &format_properties);
  ...
  s32 mip_width = (s32)texture->width;
  s32 mip_height = (s32)texture->height;
  for (u32 i = 1; i < texture->mip_levels; i++) {
    ...
    vkCmdBlitImage(command_buffer, texture->image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, texture->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_LINEAR);
...
vulkan_create_texture_image(Texture *bitmap) {
  ...
  vulkan_create_buffer(&staging_buffer, image_size, 
    VK_BUFFER_USAGE_TRANSFER_SRC_BIT, 
    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

  //void *data;
  //vkMapMemory(vk_ctx.device, staging_buffer_memory, 0, image_size, 0, &data);
  memcpy(staging_buffer.data, bitmap->data, image_size);
  vkUnmapMemory(vk_ctx.device, staging_buffer.memory);

  vulkan_create_image(bitmap, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  VkCommandBuffer command_buffer = vulkan_begin_single_time_commands(vk_ctx.command_pool);
  vulkan_transition_image_layout(command_buffer, bitmap, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  vulkan_end_single_time_commands(command_buffer, vk_ctx.command_pool, vk_ctx.graphics_queue);
  
  vulkan_copy_buffer_to_image(&staging_buffer, bitmap->image, (u32)bitmap->width, (u32)bitmap->height);
  ...
  vulkan_generate_mipmaps(bitmap);
}

internal void
vulkan_create_texture(Texture *bitmap, u32 texture_parameters) {
  if (bitmap->mip_levels == 0) {
    bitmap->mip_levels = (u32)SDL_floor(SDL_logf((float32)SDL_max(bitmap->width, bitmap->height))) + 1;
  }   
  ...
  vulkan_create_image_view(bitmap, VK_IMAGE_ASPECT_COLOR_BIT);
  vulkan_create_sampler(bitmap, texture_parameters);
}

internal u32 
vulkan_set_texture(Descriptor *desc, Texture *texture) {
